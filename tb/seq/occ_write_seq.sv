class occ_write_seq #(int DATA_WIDTH = 8) extends uvm_sequence #(write_txn#(DATA_WIDTH));
    `uvm_object_param_utils(occ_write_seq #(DATA_WIDTH))
    ...
    task body();
        ...
        for(int i = 0; i<num_txn; i++) begin
            ...
            wait_for_grant();
            a_write_item_ok: assert(req.randomize() with {req.do_write==1; req.pre_delay==0;});
            send_request(req);
            wait_for_item_done();
